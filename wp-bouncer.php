...
class WP_Bouncer
{
	...
	public function wp_enqueue_scripts()
	{
		...
		if (
			defined('WP_BOUNCER_HEARTBEAT_CHECK')
	...
	public function login_flag($redirect = true)
	{
		if (is_user_logged_in()) {
			...
			// permitir logins para níveis de usuário
			global $wpdb;

			$user = $wpdb->get_row(
				$wpdb->prepare(
					// ALTERAR O SELECT PARA O BANCO CORRETO
					"SELECT * FROM wpdn_pmpro_memberships_users WHERE user_id = %s LIMIT 1",
					$current_user->ID
				)
			);
			...
			if ($user->membership_id == 1) {
				...
				$num_allowed = apply_filters('wp_bouncer_number_simultaneous_logins', 5);
			} else {
				$num_allowed = apply_filters('wp_bouncer_number_simultaneous_logins', 10);
			}
